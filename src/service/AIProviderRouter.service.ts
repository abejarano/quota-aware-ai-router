...
export class AIProviderRouterService {
  ...
  async execute<T>(args: RouterExecuteArgs<T>): Promise<T> {
    ...
    for (const candidate of candidates) {
      ...
      try {
        ...
        const actualTokens = this.estimateTokens(
  ...
  private shouldReleaseOpenRouter(
    ...
    const primary = providerSnapshots.filter(
      (s) => s.cfg.name !== "openrouter",
    );
  ...
  private async getExternalQuota(
    ...
    return this.getValue<ProviderExternalQuota>(
      this.externalQuotaKey(provider),
    );
  }
...
`;
  ...
  private secondsUntilSliceEnd(): number {
    ...
    return Math.max(
      1,
      Math.floor((nextSlice.getTime() - now.getTime()) / 1000),
    );
  }
