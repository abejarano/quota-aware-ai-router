...
export class OpenRouterProvider implements IProxyAIProvider {
  private static _instance: OpenRouterProvider | null = null;

  private logger = console;

  static getInstance() {
    if (!this._instance) {
      this._instance = new OpenRouterProvider();
    }
  ...
  async execute(
    ...
    const providerCfg = findAIProviderByProvider("openrouter");
    const apiKey = providerCfg?.apiKey;
    if (!apiKey) {
      throw buildAIProviderError({
        provider: "OpenRouter",
        message:
          "Missing apiKey in AI_PROVIDER_CONFIG for provider 'openrouter'",
      });
    }
    ...
    if (!model) {
      throw buildAIProviderError({
        provider: "OpenRouter",
        message:
          "Missing model in AI_PROVIDER_CONFIG for provider 'openrouter'",
      });
    }
